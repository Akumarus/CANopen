...
co_res_t canopen_server_process_nmt(co_obj_t *canopen, co_msg_t *msg)
{
    assert(canopen != NULL);
    ...
    if ((node_id != canopen->node_id) && (node_id != 0))
        return CANOPEN_ERROR; // TODO Нужно вернуть, что фильтр неправильно сконфигурирован или
                              // может это норма
...
co_res_t canopen_server_transmite_bootup(co_obj_t *canopen)
{
    assert(canopen != NULL);
...
co_res_t canopen_client_transmite_nmt_cmd(co_obj_t *canopen, uint8_t node_id, canopen_nmt_cmd_t cmd)
{
    assert(canopen != NULL);
    assert(canopen->role == CANOPEN_CLIENT);
