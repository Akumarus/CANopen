...
co_res_t co_pdo_cfg_tx(co_obj_t *co, pdo_num_t pdo_num, uint8_t node_id,
                       co_msg_t *msg, uint8_t dlc)
{
    ...
    msg->type = (co->role == CANOPEN_SERVER) ? PDO_TX_TYPE(pdo_num)
                                             : PDO_RX_TYPE(pdo_num);
    msg->id = (co->role == CANOPEN_SERVER) ? PDO_TX_BASE(pdo_num)
                                           : PDO_RX_BASE(pdo_num);
    msg->id += node_id;
    msg->dlc = dlc;
...
co_res_t co_pdo_cfg_rx(co_obj_t *co, pdo_num_t pdo_num, uint8_t node_id,
                       co_hdl_t callback)
{
    assert(co != NULL);
    ...
    uint32_t id = (co->role == CANOPEN_SERVER) ? PDO_RX_BASE(pdo_num)
                                               : PDO_TX_BASE(pdo_num);
    id += node_id;
